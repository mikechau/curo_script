...
############# SELL CHECK    
  ...
  most_recent = most_recent.reverse
    ...
    if most_recent.any? {|o| o[:ticker] == trade[:ticker] && o[:qty] >= trade[:qty] && o[:date] == trade[:date]}
      ...
      matching_positions = most_recent.select {|o| o[:ticker] == trade[:ticker] && o[:qty] >= trade[:qty] && Date.parse(o[:date]) < Date.parse(trade[:date])}
      last_index = (matching_positions.count) - 1
      previous_qty = matching_positions[last_index][:qty]
      new_qty = previous_qty - trade[:qty]

      open_positions << {:date => trade[:date], :ticker => trade[:ticker], :price => matching_positions[last_index][:price], :qty => new_qty}
